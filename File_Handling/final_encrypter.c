...
void get_user_data(struct user_data *,int);
...
int check_key_validation(int attempt,struct user_data *ptr)
{
    if ( ! (ptr->key > 1000 && ptr->key < 9999)  )
    {
        if (attempt > 5)
        {
            ...
            rename("C:\\ENCRYPTER\\hid.txt","C:\\ENCRYPTER\\.hid");
            printf("Aborted !\n");
            exit(0);
        }
...
void get_user_data(struct user_data *User,int isAgain)
{
...
int is_file_already_encrypted(struct user_data *u)
{
    ...
    if ( (data_file = fopen("C:\\ENCRYPTER\\.hid","r")) == NULL )
    {
...
void save_info(const char *ch ,struct user_data *u)
{
    ...
    fp = fopen("C:\\ENCRYPTER\\store_info.txt","a");
    if (fp == NULL)
    {
...
void store_user_record_in_file(struct user_data *u)
{
    ...
    if(( data_file = fopen("C:\\ENCRYPTER\\.hid","a") )== NULL )
    {
        sprintf(error_msg," %s : %s ","Main File",strerror(errno));
        message_file_opening_error(error_msg);
        exit(0);
...
void remove_user_record(struct user_data *u)
{
    ...
    if ( (temp_file = fopen("C:\\ENCRYPTER\\temp.txt","w") ) == NULL )
    {
    ...
    remove("C:\\ENCRYPTER\\.hid");
    rename("C:\\ENCRYPTER\\temp.txt","C:\\ENCRYPTER\\.hid");
...
void encrypt_folder()
{
   ...
   if( check_username_and_key(&user) )
   {
    ...
    while(i >= 0)
    {
        printf("\n\n\t\tEncryption is starting in (%d) sec...", i--);
        printf("\x1b[1F");
        printf("\x1b[1F");
        Sleep(1000);
    }
...
void decrypt_folder()
{
    ...
    while(i >= 0)
    {
        printf("\n\n\t\tDecryption is starting in (%d) sec...", i--);
        printf("\x1b[1F");
        printf("\x1b[1F");
        Sleep(1000);
    }
...
void show_all_entries(void)
{
    ...
    data_file = fopen("C:\\ENCRYPTER\\.hid","r");
    if (data_file == NULL)
    {
        sprintf(error_msg," %s : %s ","Main File",strerror(errno));
        message_file_opening_error(error_msg);
    ...
    printf("\n\t+----------+--------------+-------------------------------------");
    printf("\n\t| Password |   Username   | File / Folder path ...");
    printf("\n\t+----------+--------------+-------------------------------------");
    while( (fscanf(data_file,"%s %d %s",user_name,&key,dir_name)) != EOF )
    {
        ...
        printf("\n\t| %-8d | %-12s | %s ",key,user_name,dir_name);
...
int check_directory_setup()
{
    DIR *dir;

    dir = opendir("C:\\ENCRYPTER");
    if(dir == NULL){
        return 0;
    }
    else{
...
{
    mkdir("C:\\ENCRYPTER");
    FILE *fr = fopen("C:\\ENCRYPTER\\.hid", "w");
    FILE *store_info = fopen("C:\\ENCRYPTER\\store_info.txt", "w");
    fclose(store_info);
    fclose(fr);
}
